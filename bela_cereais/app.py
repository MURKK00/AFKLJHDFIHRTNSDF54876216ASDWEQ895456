...
def verificar_senha():
    if st.session_state["senha_digitada"] == "bela2026": 
...
anos_disp = sorted(df_lucro['Ano'].unique().tolist()) if 'Ano' in df_lucro.columns else [2025]
...
df_rf = filtrar_d(df_rec_fin) # FILTRA AS RECEITAS PARA NÃO ZERAR

# CALCULA COM AS TRÊS BASES JUNTAS
kpis  = calcular_kpis(df_lf, df_df, df_rf)
