...
def render(kpis: dict):
    ...
    with col_dre:
        cor_final = "#2ECC71" if pos else "#E74C3C"
        ...
        html_dre = (
            ...
            f"<span style='color:#C8CAD4;font-size:15px;font-weight:500'>(-) Custo Operacional (Fethab, Terceiros, etc)</span>"
            f"<span style='color:#E74C3C;font-size:16px;font-weight:500;font-family:\"Montserrat\",sans-serif'>{br(-k.get('custo_operacional', 0))}</span>"
            "</div>"
            ...
            f"<div style='display:flex;justify-content:space-between;align-items:center;padding:14px 20px;margin-bottom:10px'>"
            f"<span style='color:#C8CAD4;font-size:15px;font-weight:500'>(+) Receitas Extras (Financeiras/ICMS/Outros)</span>"
            f"<span style='color:#2ECC71;font-size:16px;font-weight:500;font-family:\"Montserrat\",sans-serif'>{br(k.get('receita_financeira', 0))}</span>"
            "</div>"
